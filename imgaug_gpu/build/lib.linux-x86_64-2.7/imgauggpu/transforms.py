...
class Sometimes(object):
    ...
    def __init__(self, transform, p ):
        ...
        self.p = ia.parameters.Binomial(1 - p)
    ...
    def __call__(self, images):
        ...
        if samples_true_index.size > 0:
            images_to_transform = torch.index_select(images, 0, torch.cuda.LongTensor(samples_true_index))
            if samples_false_index.size > 0:
                ...
                images_not_to_transform = torch.cuda.FloatTensor()
                return torch.cat((self.transform(images_to_transform), images_not_to_transform), 0)
...
def Dropout(p=0, per_channel=False, name=None, deterministic=False,
    ...
    return MultiplyElementwise(p, per_channel=per_channel, name=name, deterministic=deterministic, random_state=random_state)
