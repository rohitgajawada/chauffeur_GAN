...
def execute(gpu, exp_batch, exp_alias):
    ...
    for data in data_loader:
        ...
        if iteration % 1000 == 0:
            vutils.save_image(torch.cat((inputs[:3] + 0.5, fake_inputs[:3] + 0.5), 0).cpu().data, './imgs/' + str(iteration) + 'real_samples.png', normalize=True)
            coil_logger.add_image("Images", torch.cat((inputs[:3] + 0.5, fake_inputs[:3]), 0), iteration)
