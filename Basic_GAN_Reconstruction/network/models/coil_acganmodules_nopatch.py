...
class _netG(nn.Module):
    def __init__(self, loss='NORMAL', skip=False):
...
class _netD(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=4, norm_layer=torch.nn.InstanceNorm2d):
        ...
        aux_sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]


        self.shared = nn.Sequential(*shared_sequence)
        self.main_unshared = nn.Sequential(*main_sequence)
        self.aux_unshared = nn.Sequential(*aux_sequence)

    def forward(self, input):
        shared_out = self.shared(input)

        output = self.main_unshared(shared_out)
        steer = self.aux_unshared(shared_out)
        return output, steer.view(-1)
