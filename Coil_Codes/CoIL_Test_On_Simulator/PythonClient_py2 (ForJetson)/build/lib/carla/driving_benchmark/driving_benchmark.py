...
class DrivingBenchmark(object):
	...
	def benchmark_agent(self, experiment_suite, agent, client):
		...
		for experiment in experiment_suite.get_experiments()[int(start_experiment):]:
			...
			print("Poses len ", len(experiment.poses[start_pose:]))
			for pose in experiment.poses[start_pose:]:
				print("Another pose")
				for rep in range(experiment.repetitions):

					start_index = pose[0]#0#random.randint(0,11)
					end_index = pose[1]#23#random.randint(12,23)
					...
					time_out = 1000#experiment_suite.calculate_time_out(
