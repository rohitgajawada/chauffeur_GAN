...
_g_conf.LOG_IMAGE_WRITING_FREQUENCY = 20
...
_g_conf.TEST_SCHEDULE = range(0, 2000, 2000)
...
def set_type_of_process(process_type, param=None):
    ...
    if process_type == "train":
        if not os.path.exists(os.path.join('/datatmp/Datasets/rohitgan/_logs', _g_conf.EXPERIMENT_BATCH_NAME,
                                        _g_conf.EXPERIMENT_NAME,
                                        'checkpoints') ):
                os.mkdir(os.path.join('/datatmp/Datasets/rohitgan/_logs', _g_conf.EXPERIMENT_BATCH_NAME,
                                            _g_conf.EXPERIMENT_NAME,
                                            'checkpoints'))
