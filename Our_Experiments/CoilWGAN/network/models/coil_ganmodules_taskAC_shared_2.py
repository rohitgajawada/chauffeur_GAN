...
#TODO: decrease amount of common layers for discriminator
...
class _netD(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=4, norm_layer=torch.nn.InstanceNorm2d, loss='LSGAN'):
        ...
        self.main_model = nn.Sequential(*main_sequence)
        self.aux_model = nn.Sequential(*aux_sequence)

    def forward(self, input):
        shared_out = self.shared_model(input)
        bin_out = self.main_model(shared_out)
        aux_out = self.aux_model(shared_out)

        return bin_out, aux_out
