...
import network.models.coil_ganmodules_taskAC_shared as ganmodels_taskAC_shared
...
def calc_gradient_penalty(netD, inputs, fake_inputs, flag):
    ...
    if flag == "recon":
        pred_hat, __ = netD(x_hat)
    elif flag == "da":
        __, pred_hat = netD(x_hat)
...
def execute(gpu, exp_batch, exp_alias):
    ...
    for data in data_loader:
        ...
        if iteration % 100 == 0:
            imgs_to_save = torch.cat((inputs[:1], src_fake_inputs[:1], tgt_imgs[:1], tgt_fake_inputs[:1]), 0).cpu().data
        ...
        gradient_penalty_src = calc_gradient_penalty(netD, inputs, src_fake_inputs_forD, "recon")
        lossD_bin_src = torch.mean(outputsD_fake_src_bin - outputsD_real_src_bin) + gradient_penalty_src

        gradient_penalty_tgt = calc_gradient_penalty(netD, tgt_imgs, tgt_fake_inputs_forD, "recon")
        ...
        #####Task network updates##########################
            ...
            gradient_penalty_da_1 = calc_gradient_penalty(netD, tgt_imgs, src_fake_inputs_forD, "da")
            lossF_da_1 = torch.mean(outputsD_fake_src_da - outputsD_real_tgt_da) + gradient_penalty_da_1

            gradient_penalty_da_2 = calc_gradient_penalty(netD, inputs, tgt_fake_inputs_forD, "da")
            lossF_da_2 = torch.mean(outputsD_fake_tgt_da - outputsD_real_src_da) + gradient_penalty_da_2

            lossF_da = 0.5 * (lossF_da_1 + lossF_da_2)
            lossF = (lossF_task + task_adv_weight * lossF_da) / (1.0 + task_adv_weight)
