...
def execute(gpu, exp_batch, exp_alias):
    ...
    for data in data_loader:
        ...
        outputsD_fake_tgt_img = netD_img(src_img_fake.detach())

        gradient_penalty = calc_gradient_penalty(netD_img, src_img_fake, tgt_imgs)
        lossD_img = torch.mean(outputsD_real_tgt_img - outputsD_fake_src_img) + gradient_penalty
        ...
        #####Generator updates#######
            ...
            lossG = outputsD_fake_src_img.mean() - outputsD_real_tgt_img.mean()
        ...
        #####Task network updates##########################
            ...
            lossF_adv_bin = 0 #netD_bin(src_embed_inputs).mean() - netD_bin(tgt_embed_inputs).mean()
            lossF_adv_img = outputsD_fake_src_img.mean() - outputsD_real_tgt_img.mean()
            lossF_adv = (lossF_adv_bin + lossF_adv_img)
