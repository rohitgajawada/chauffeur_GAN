...
def execute(gpu, exp_batch, exp_alias):
    ...
    optimD_img = torch.optim.Adam(netD_img.parameters(), lr=g_conf.LR_D, betas=(0.5, 0.999))
    optimG = torch.optim.Adam(netG.parameters(), lr=g_conf.LR_D, betas=(0.5, 0.999))
    if g_conf.TYPE =='task':
    ...
    for data in data_loader:
        ...
        if iteration % 100 == 0:
            imgs_to_save = torch.cat((inputs[:1] + val, src_img_fake[:1] + val, tgt_imgs[:1] + val, tgt_img_fake[:1] + val), 0).cpu().data
        ...
        gradient_penalty_src = calc_gradient_penalty(netD_img, inputs, src_img_fake)
        lossD_img_src = torch.mean(outputsD_fake_src_img - outputsD_real_src_img) + gradient_penalty_src

        gradient_penalty_tgt = calc_gradient_penalty(netD_img, tgt_imgs, tgt_img_fake)
        lossD_img_tgt = torch.mean(outputsD_fake_tgt_img - outputsD_real_tgt_img) + gradient_penalty_tgt
        ...
        #####Generator updates#######
            ...
            lossG_adv = 0.5 * (- 1.0 * outputsD_fake_src_img.mean() - 1.0 * outputsD_fake_tgt_img.mean())
        ...
        #####Task network updates##########################
            ...
            lossF_adv_bin = netD_bin(src_embed_inputs).mean() - netD_bin(tgt_embed_inputs).mean()
            lossF_adv_img = outputsD_fake_src_img.mean() - outputsD_real_tgt_img.mean()
