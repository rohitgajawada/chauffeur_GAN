...
import network.models.coil_ganmodules_taskAC as ganmodels_task
...
def execute(gpu, exp_batch, exp_alias):
    ...
    for data in data_loader:
        ...
        if g_conf.LABSMOOTH:
            label_real_noise = torch.rand(labsize.size()) * 0.1 #Label smoothing
            label_fake_noise = torch.rand(labsize.size()) * 0.1

        labels_src_fake = torch.zeros(labsize).type(torch.LongTensor) + 1 #Fake labels
        labels_src_fake = Variable(labels_src_fake).cuda()

        ##source real
        outputsD_src_real_forD = netD(inputs) # Pass real domain image here
        labels_src_real = torch.zeros(labsize).type(torch.LongTensor) + 2 #Real labels
        ...
        labels_tgt_real = torch.zeros(labsize).type(torch.LongTensor) + 4 #Real labels
        ...
        lossD = (lossD_src_real + lossD_src_fake + lossD_tgt_real + lossD_tgt_fake) * 0.25
