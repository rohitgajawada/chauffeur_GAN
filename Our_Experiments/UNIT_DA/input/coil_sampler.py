...
class PreSplittedSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if rank_keys == 2:
            ...
            weights = torch.DoubleTensor([1.0/float(len(self.keys))]*len(self.keys))

            idx = torch.multinomial(weights, self.config['max_iter'], True)
            idx = idx.tolist()
            ...
            idx = torch.multinomial(weights, self.config['max_iter'], True)
            idx = idx.tolist()
            weights = torch.DoubleTensor([1.0 / float(len(self.keys[0]))] * len(self.keys[0]))
            idy = torch.multinomial(weights, self.config['max_iter'], True)
            idy = idy.tolist()
    ...
    def __len__(self):
        return self.config['max_iter'] * self.config['batch_size']
...
class BatchSequenceSampler(object):
    ...
    def __init__(self, keys, batch_size, sequence_size, sequence_stride, config, drop_last=True):
        sampler = PreSplittedSampler(keys, config)
