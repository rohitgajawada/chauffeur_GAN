...
class CoILDataset(Dataset):
    ...
    def __getitem__(self, used_ids):
        ...
        sensors_gconf = {'rgb': (3, 128, 128)}
        number_frames_fusion_gconf = 1
        ...
        for sensor_name, sensor_size in sensors_gconf.items():
            sensor_data = np.zeros(
                (number_of_position, sensor_size[0], sensor_size[1],
                 sensor_size[2] * number_frames_fusion_gconf),
                 dtype='float32'
            )
        ...
        for sensor_name, sensor_size in sensors_gconf.items():
            ...
            for chosen_key in used_ids:
                ...
                for i in range(number_frames_fusion_gconf):
    ...
    def pre_load_hdf5_files(self, path_for_files):
        ...
        measurement_keys = {'targets': (31)}
        meas_names = list(measurement_keys.keys())
        # take the names for all sensors
        sensors_names = list(sensors_gconf.keys())
    ...
    def extract_inputs(self, float_data):
        ...
        inputs_gconf = ['speed_module']
        inputs_vec = []
        for input_name in inputs_gconf:
