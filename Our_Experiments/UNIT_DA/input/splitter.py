...
def order_sequence(steerings, keys_sequence):
    ...
    for i in keys_sequence:
        sampled_sequence = steerings[(i):(i + 1)]
...
def select_data_sequence(control, selected_data):
    ...
    while count * 1 <= (
        ...
        for iter_sequence in range((count * 1),
                                   (count * 1) +
                                   1):
            ...
            if (eliminated_positions) > 1/2:
                del_pos.append(count * 1)
                break_sequence = True
                break
...
def control_steer_split(float_data, meta_data):
    ...
    keys = range(0, len(steerings) - 1)

    LABELS_DIVISION = [[2]]
    splitted_labels = label_split(labels, keys, LABELS_DIVISION)
